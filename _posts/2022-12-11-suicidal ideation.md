...
결과적으로 유의미한 변인을 정리하면 다음과 같다. White matter 주요 변인을 살펴보면, emotion과 관련된 뇌 영역(ex. amygdala, ACC...)이나 보상 체계와 관련된 뇌 영역(ex. insula...) 등의 연결성이 자살 사고를 유의미하게 예측함을 알 수 있다. 특히, amygdala의 높은 연결성이 자살 사고를 유의미하게 예측하는 것은, 자살 사고를 하는 아동 청소년들이 감정 변화에 더욱 민감하기 때문일 수 있음을 시사한다. 즉, 자살 사고가 뇌의 구조적, 기능적 취약성과 연결될 수 있다는 것이다. <br/>
...
<br/>
먼저, 아동 청소년(10~20세로 가정) 전수에 대해 CBCL을 실시했을 때 필요한 예산은 153억이다. 다음으로, CBCL을 통해 자살 사고를 예측하여, 고위험군으로 분류된 아동 청소년들에 대해 심리상담을 진행한다면 주 1회 한 시간, 총 6개월의 기간으로 계산했을 때 6,720억이 필요하다. 다만, 고위험군 심리상담 비용이 국가 예산에 비해 지나치게 큰 비중을 차지한다는 점을 고려했을 때, 소득분위 혹은 가정 경제 상황을 고려하여 지원이 필요한 경우에만 국가가 그 비용을 부담하고, 그렇지 않은 경우엔 각 가정에서 부담하도록 할 수 있을 것이다.
##### **예방 예산**
<br/>
![prevent](https://user-images.githubusercontent.com/114209766/206894688-d00aec0c-886d-4c14-ab93-bd523bf7abe5.png)
<br/>
다음으로, 예방 방안 실현을 위한 예산이다. 가장 먼저, CBCL for friends를 개발하기 위한 초기 비용이 필요하다. 정확히 필요한 예산을 가늠할 수 없어, 2022년 과학기술부 뇌과학 분야의 연구과제 평균 연구비로 근사했을 때 약 7,000만 원을 배정하였다. 다음으로, 한국형 바이오뱅크 구축을 위한 예산이다. 머신 러닝의 데이터로 사용했던 ABCD의 사례를 보면, 미국의 아동 중 약 11,880명을 대상으로 연구가 진행되고 있다. 미국의 10~19세 아동 청소년 전체 인구 대비 ABCD 연구에 참여하는 아동 청소년의 비율을 고려했을 때, 한국에서 비슷한 규모의 바이오뱅크를 구축하기 위해서는 약 1,400명의 데이터가 필요하다. 앞서 머신 러닝 결과, 자살 사고가 있는 아동 청소년을 예측하기 위한 최적의 모델로 “CBCL+dMRI+PRS”를 선정한 바 있다. 이러한 결과에 따라, 한국형 바이오뱅크를 구축할 때 dMRI와 유전자 검사를 필수 항목으로 채택할 것을 제안한다. 유전자 검사의 경우 매년 종단적으로 검사할 필요가 없기 때문에 인당 약 10만 원은 초기 비용으로 분류하였다. 이외에, dMRI의 경우 매년 측정해야 하기 때문에, 연간 약 7억의 비용이 필요할 것이다.
이외에, fMRI와 sMRI는 부수적인 요소로써, 자살 사고가 있는 아동을 예측해내기 위한 최적의 모델에 포함되지는 않았으나, 만약 다른 분야의 연구 기반을 위해 필요한 경우 추가적으로 수집할 수 있는 정보들이다. 이러한 경우, 종단 연구가 필요한 타 연구 분야 혹은 기관과 협력하여 데이터를 수집할 수 있을 것이라 기대한다. <br/>
<br/>
## **앞으로 나아가야 할 방향**
지금까지, 머신 러닝을 기반으로 자살 사고가 있는 아동 청소년을 예측해낼 수 있는 최적의 모델을 찾아내고, 이에 근거한 치료 및 예방 방안을 제시하였다. 서론에서 언급한 바와 같이, 대한민국 아동 청소년의 자살 문제는 다른 국가들에 비해서도 심각한 수준이다. 이러한 상황에서, 지금까지 제시한 CBCL, dMRI, PRS 데이터를 활용하여 자살 사고가 있는 아동 청소년을 찾아내고 예방하는 것이 자살률을 유의미하게 낮출 수 있을 것이라 기대한다. 다만, 더 중요한 것은 주변의 아동 청소년, 자녀, 친구 등에 관심을 기울이고, 그들이 내는 목소리에 귀를 기울이는 것이다. 주변에 관심을 가지는 것에 더해, CBCL, dMRI, PRS 데이터를 기반으로 한 예측이 이루어진다면, 뇌데사씨가 제안한 “자살률 0% 달성”에 가까워지리라 생각한다. 
